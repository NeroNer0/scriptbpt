...
const expiredCheck = (arbakti, _dir) => {
    setInterval(() => {
        let position = null
        Object.keys(_dir).forEach((i) => {
        ...
        if (position !== null) {
            ...
            arbakti.sendMessage(_dir[position].id, { text: `Masa sewa di grup ini telah habis, bot otomatis keluar!` })
            .then( async res => {
              await arbakti.groupLeave(_dir[position].id)
...
function changeSetBot(teks, groupID, _db) {
    let position = null
    Object.keys(_db).forEach((x) => {
        if (_db[x].id === groupID) {
            position = x
...
function addSetBot (teks, groupID, _db){
    const obj_add = {
        id: groupID,
        text: teks
    }
...
function removeSetBot (groupID, _db) {
    let position = null
    Object.keys(_db).forEach((x) => {
        if (_db[x].id === groupID) {
            position = x
...
function getTextSetBot(groupID, _db){
    let position = null
    Object.keys(_db).forEach((x) => {
        if (_db[x].id === groupID) {
            position = x
